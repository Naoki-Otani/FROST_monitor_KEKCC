...
//root -l -q 'covertlightyield.C("input.root", "chmap.txt", "refgain.csv")'
...
static const int SAMPLING_FIRST_BANCH = 10; // 1バンチ目の積分開始サンプル番号（実際のビームデータで調整する必要がある）
static const int ADC_THRESHOLD = 520; //leading_fromadc, trailing_fromadcの閾値
static const double BUNCH_INTERVAL = 43.5; //bunch間隔[Smapling]

static const int CH_PER_PLANE = 32;
static const int NOUT         = 272;
static const int NBUNCH       = 8;   // # of bunchs
...
void convertlightyield_rayraw_(const char* infile, const char* outfile, const char* chmap_file, const char* referencegain, const char* rayrawcalib)
{
  ...
  auto   out_leading_fromadc  = new std::vector<std::vector<double>>(); //ADC（波形）で計算したleadingのサンプリング
  ...
  for (Long64_t ie = 0; ie < nent; ++ie) {
    ...
    out_leading_fromadc->assign(NOUT, std::vector<double>());   // 空ベクタで NOUT 分
    ...
    if (waveform) {
      ...
      for (int ch = 0; ch < nChAll; ++ch) {
        ...
        for(int bunch=0; bunch<NBUNCH; ++bunch){
          out_lightyield[idx][bunch] = (adcint[bunch] - zero_pe * (double)(INT_RANGE)/(double)intrange_calib) * (m1_ref_base - m0_ref_base) / ((one_pe_base - zero_pe_base)*(m1_ref - m0_ref));
        }
...
void convertlightyield_rayraw(const char* infile, const char* chmap_file, const char* refgainfile)
{
  std::string path_beamdata = "/group/nu/ninja/work/otani/FROST_beamdata/test/";
  TString filename_inroot = TString(path_beamdata) + "rootfile/" + infile + ".root";
  TString filename_outroot = TString(path_beamdata) + "rootfile_aftercalib/" + infile + "_lightyield.root";
  TString filename_chmap = TString(path_beamdata) + "chmap/" + chmap_file + ".txt";
  TString filename_refgain = TString(path_beamdata) + "calibration/ReferenceGain/" + refgainfile + ".csv";
  TString filename_calib = TString(path_beamdata) + "calibration/calibresult/calib_" + infile + ".csv";
