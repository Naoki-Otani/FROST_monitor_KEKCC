...
void calibrayraw_(TString filename, TString chmapPath, TString outputcsv, TString outputplot) {
  ...
  const Int_t BL_START_MAX = 370;      // 開始点の上限（0,5,...,70）
  const Int_t BL_STEP = 10;            // スライド刻み
...
void calibrayraw(const char* infile, const char* chmap_file)
{
  ...
  TString filename_outcsv = TString(path_beamdata) + "calibration/calibresult/calib_" + infile + ".csv";
  TString filename_outplot = TString(path_beamdata) + "calibration/plot/calib_" + infile;
